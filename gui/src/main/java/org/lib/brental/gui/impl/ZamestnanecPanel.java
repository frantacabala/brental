 ...
 * @author john
 */
public class ZamestnanecPanel extends JPanel {
    ...
    public ZamestnanecPanel(Color color){
        setOpaque(true);
        setBackground(color);
        init_ui();
    }
...
//        gbc.gridx ++;
        ...
        JScrollPane scp;
        add(scp = new JScrollPane(tbl = new JTable(zm = new ZamestnanecModel())), BorderLayout.CENTER);
        MainFrame.addRefreshable(zm);
    ...
    class ZamRender extends BasicComboBoxRenderer {
        ...
        public Component getListCellRendererComponent(JList list, Object value,
            ...
            if (value != null) {
                Zamestnanec zam = (Zamestnanec) value;
                setText(zam.getFunkce().toUpperCase());
            }
            if (index == -1) {
                Zamestnanec zam = (Zamestnanec) value;
                setText("" + zam.getFunkce());
            }
            return this;
        }
    }

    public DefaultComboBoxModel<Zamestnanec> getAutaSPZ() {
        DefaultComboBoxModel<Zamestnanec> model = new DefaultComboBoxModel<Zamestnanec>();
        Collection<Zamestnanec> z = null;
        try {
            z = BrentalFacadeService.getDefault().getZamestnance();
        } catch (BRentalException ex) {
            Logger.getLogger(AutoPanel.class.getName()).log(Level.SEVERE, null, ex);
        }
        for (Zamestnanec zam : z) {
            model.addElement(zam);
        }
        return model;
    ...
    Collection<Zamestnanec> getSelectedZamestnaneci(){
        int[] rows = tbl.getSelectedRows();
        return zm.getZamestnanci(rows);
    }
    ...
    public Zamestnanec getSelectedZamestnanec(){
       int row = tbl.getSelectedRow();
        ...
        if(row == -1)
            return null;
        return new ArrayList<Zamestnanec>(zm.getZamestnanci(new int[]{row})).get(0);
