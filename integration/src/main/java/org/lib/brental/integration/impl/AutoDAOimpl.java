...
class AutoDAOimpl implements AutoDAO {
    ...
    public AutoDAOimpl() {
        try{
        create("1232-sds",3,1423,new Date(),new Date());
        create("asdasd32-sds",3,1423,new Date(),new Date());
        create("asdasd32-sds",3,1423,new Date(),new Date());
    ...
    public Auto create(String spz, int kapacita,int cena_zakoupeni, Date datum_zakoupeni, Date datum_tk) throws BRentalException{
        Auto fak = new Auto(new AutoId(idCount++), spz,kapacita,cena_zakoupeni,datum_zakoupeni,datum_tk);
