 ...
 * @author danecek
 */
public final class EditZakaznikDialog extends AbstractBrentalDialog {
    ...
    public EditZakaznikDialog(Zakaznik a) {
        ...
        op.setText(Integer.toString(a.getCislo_op()));
    ...
    public void okAction() {
        try {
            ...
            BrentalFacadeService.getDefault().aktualizujZakaznika(new Zakaznik(zak.getId(), jmeno.getText(), prijimeni.getText(), new Adresa(ulice.getText(), mesto.getText(), Integer.parseInt(psc.getText())), Integer.parseInt(op.getText())));
            MainFrame.getInstance().refresh();
        } catch (BRentalException ex) {
            Logger.getLogger(EditZakaznikDialog.class.getName()).log(Level.SEVERE, null, ex);
        }
    }
}
